...
class AgentDefendCenter(agent.Agent):

    def __init__(self, scenario_name, agent_identifier, temperature=1.0, image_dim=80, lr=0.001):
        super(AgentDefendCenter, self).__init__(scenario_name, agent_identifier, temperature, image_dim, lr)
        self.cnn = cnn_agent_second_input.CnnTwoInput(number_actions=self.nb_available_buttons, image_dim=image_dim, linear_input=3)
    ...
    def read_game_data(self, game):
        ...
        self.current_ammo = game.get_game_variable(viz.GameVariable.AMMO2)
        step = state.number
        self.linear_input = np.array([[self.current_health, self.current_ammo, step]])
