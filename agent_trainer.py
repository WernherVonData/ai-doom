...
class AgentTrainer:
    def __init__(self, agent_name, scenario_name, temperature=1.0, memory_capacity=100000, lr=0.001):
        self.scenario_path, self.nb_available_buttons = utils.get_path_and_number_of_actions_to_scenario(
        ...
        if agent_name == "cnn_image_linear":
            self.cnn = cnn_agent_second_input.CnnTwoInput(number_actions=self.nb_available_buttons, image_dim=image_dim,
                                                          linear_input=4)
    ...
    def train(self, step_reader, make_action, calculate_reward, generate_history, eligibility_trace, starting_epoch=1,
              nb_epochs=100, batch_size=128, n_step=10, nb_steps=1000, gamma=0.99):
        """
        ...
        :param make_action: receives ai object and the results of the step_reader and this object ai object and must return action.
        ...
        while True:
            ...
            while True:
                ...
                history.append(generate_history(action=action, game_finished=game.is_episode_finished()))
            ...
            for batch in self.memory.sample_batch(batch_size):
                image_inputs, linear_inputs, targets = eligibility_trace(cnn=self.cnn, batch=batch, gamma=gamma)
            ...
            if epoch % 10 == 0:
                ...
                utils.save(model_file, self.cnn, self.optimizer)
            if epoch == nb_epochs:
